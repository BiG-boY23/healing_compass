...
$barangay  = $_SESSION['barangay'] ?? ''; // This should be set in AuthController or fetched if missing
...
try {
    ...
    // Module A Table: Healers from this barangay
    $localHealers = $pdo->prepare("SELECT * FROM healers WHERE barangay = ? ORDER BY is_verified ASC, full_name ASC");
    $localHealers->execute([$barangay]);
    $localHealerList = $localHealers->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    ...
    <style>
        ...
        .bhw-sidebar-link.active {
            background: var(--bhw-primary) !important;
...
<div class="container-fluid p-0">
    <div class="row g-0">
        <!-- Sidebar -->
        <nav class="col-md-3 col-lg-2 sidebar p-4" style="background: white; border-right: 1px solid #e3e8ef;">
            ...
            <div class="nav-menu">
                <a href="bhw-dashboard.php" class="sidebar-link active bhw-sidebar-link">
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-5 py-5 min-vh-100">
            ...
            <div class="row g-4">
                <!-- Module A: Healer Verification -->
                <div class="col-lg-8">
                    <div class="card border-0 rounded-4 shadow-sm p-4">
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            <h5 class="fw-bold text-dark mb-0">Module A: Local Healer Verification</h5>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover align-middle">
                                ...
                                <tbody>
                                    <?php if (empty($localHealerList)): ?>
                                        <tr><td colspan="4" class="text-center py-4 text-muted">No healers found in this barangay.</td></tr>
                                    <?php else: ?>
                                        <?php foreach ($localHealerList as $lh): ?>
                                        <tr>
                                            <td>
                                                <div class="fw-bold"><?= htmlspecialchars($lh['full_name']) ?></div>
                                                <small class="text-muted"><?= htmlspecialchars($lh['location_name']) ?></small>
                                            </td>
                                            <td class="small"><?= htmlspecialchars($lh['specialization']) ?></td>
                                            <td>
                                                <?php if ($lh['is_verified']): ?>
                                                    <span class="badge rounded-pill bhw-badge-verified px-3 py-2">Verified</span>
                                                <?php else: ?>
                                                    <span class="badge rounded-pill bhw-badge-pending px-3 py-2">Pending Review</span>
                                                <?php endif; ?>
                                            </td>
                                            <td class="text-center">
                                                <?php if (!$lh['is_verified']): ?>
                                                    <form method="POST" onsubmit="return confirm('Verify this healer for your barangay?')">
                ...
                <div class="col-lg-4">
                    ...
                    <div class="card border-0 rounded-4 shadow-sm p-4 mb-4" style="background: var(--bhw-text); color: white;">
                        ...
                        <div class="d-grid gap-2">
                            <button class="btn btn-light rounded-pill text-start py-2 px-3 fw-bold small d-flex align-items-center justify-content-between" onclick="window.location.href='admin-healers.php'">
                            ...
                            <button class="btn btn-light rounded-pill text-start py-2 px-3 fw-bold small d-flex align-items-center justify-content-between" onclick="window.location.href='admin-plants.php'">
                    ...
                    <!-- Module C: Local Health Alerts -->
                    <div class="card border-0 rounded-4 shadow-sm p-4">
                        <h6 class="fw-bold text-dark mb-4">Module C: Barangay Health Alerts</h6>
                        ...
                        <?php else: ?>
                            ...
                            <?php endforeach; ?>
                        <?php endif; ?>
                        <button class="btn btn-link text-primary fw-bold small mt-2 p-0 text-decoration-none">+ Create New Alert</button>
                    </div>
                </div>
                ...
                <div class="col-12 mt-2">
                    <div class="card border-0 rounded-4 shadow-sm p-4">
                        <h6 class="fw-bold text-dark mb-4">Module B: Local Botanical Inventory</h6>
                            ...
                            <?php else: ?>
                                ...
                                <div class="col">
                                    <div class="text-center p-3 rounded-4" style="background: #f8f9fa; border: 1px solid #eee;">
                                        <div class="mb-2 mx-auto" style="width: 50px; height: 50px; background: white; border-radius: 12px; display: flex; align-items: center; justify-content: center;">
                                            <i data-lucide="leaf" class="text-success" style="width: 20px;"></i>
                                        </div>
                                        <div class="fw-bold " style="font-size: 0.8rem;"><?= htmlspecialchars($p['plant_name']) ?></div>
                                        <small class="text-muted" style="font-size: 0.7rem;"><?= htmlspecialchars($p['illness_treated']) ?></small>
                                    </div>
                                </div>
                                <?php endforeach; ?>
                            <?php endif; ?>
